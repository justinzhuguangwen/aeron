...
static bool can_terminate(const aeron_consensus_module_agent_t *agent,
                          int64_t now_ns);
static void setup_cluster_termination(aeron_consensus_module_agent_t *agent,
                                      int64_t position, int64_t now_ns);
static void terminate_on_service_ack(aeron_consensus_module_agent_t *agent,
                                     int64_t log_position, int64_t now_ns);

/* -----------------------------------------------------------------------
 * Archive recording-signal consumer (installed on the CM's archive ctx).
 *
 * Captures the log's recording_id the first time the archive reports a
 * START signal whose subscription_id matches the recording subscription
 * the CM just created; records a flag on STOP so the next do_work tick
 * can trigger enterElection(). Mirrors Java ConsensusModuleAgent.
 * ----------------------------------------------------------------------- */
static void cm_on_recording_signal(
    aeron_archive_recording_signal_t *signal, void *clientd) {
  aeron_consensus_module_agent_t *agent =
      (aeron_consensus_module_agent_t *)clientd;
  if (NULL == agent || NULL == signal) {
    return;
  }
  const int32_t code = signal->recording_signal_code;
  if (AERON_ARCHIVE_CLIENT_RECORDING_SIGNAL_START == code) {
    if (agent->log_recording_id < 0 &&
        agent->log_subscription_id == signal->subscription_id) {
      agent->log_recording_id = signal->recording_id;
      /* Flush the deferred recording_log write now that we have the id.
       * Mirrors the effect of Java's synchronous tryCreateAppendPosition
       * loop in joinLogAsLeader: recording_log_append_term now sees a
       * valid recording_id. */
      aeron_consensus_module_agent_update_recording_log(agent, aeron_nano_clock());
    }
  } else if (AERON_ARCHIVE_CLIENT_RECORDING_SIGNAL_STOP == code) {
    if (signal->recording_id == agent->log_recording_id &&
        NULL == agent->election && AERON_CM_STATE_ACTIVE == agent->state) {
      agent->log_recording_stopped_unexpectedly = true;
    }
  }
}
 ...
 * ----------------------------------------------------------------------- */
static int64_t
pending_service_message_log_append(void *clientd, int64_t leadership_term_id,
                                   int64_t cluster_session_id,
                                   int64_t timestamp, uint8_t *buffer,
                                   int payload_offset, int payload_length) {
  ...
  return aeron_cluster_log_publisher_append_session_message(
      pub, cluster_session_id, timestamp, buffer + payload_offset,
      (size_t)payload_length);
}
static void do_termination_as_follower(aeron_consensus_module_agent_t *agent,
...
static int add_exclusive_pub(aeron_t *aeron,
  ...
  if (aeron_async_add_exclusive_publication(&async, aeron, channel, stream_id) <
      0) {
  ...
  int rc = 0;
  do {
    rc = aeron_async_add_exclusive_publication_poll(pub, async);
  } while (0 == rc);
  return rc < 0 ? -1 : 0;
}
...
static int add_sub(aeron_t *aeron, aeron_subscription_t **sub,
  ...
  if (aeron_async_add_subscription(&async, aeron, channel, stream_id, NULL,
                                   NULL, NULL, NULL) < 0) {
  ...
  int rc = 0;
  do {
    rc = aeron_async_add_subscription_poll(sub, async);
  } while (0 == rc);
  return rc < 0 ? -1 : 0;
}
 ...
 * Lifecycle
 * ----------------------------------------------------------------------- */
int aeron_consensus_module_agent_create(aeron_consensus_module_agent_t **agent,
  ...
  a->log_recording_stopped_unexpectedly = false;
  a->app_version = ctx->app_version;
  a->protocol_version = aeron_semantic_version_compose(
      AERON_CM_PROTOCOL_MAJOR_VERSION, AERON_CM_PROTOCOL_MINOR_VERSION,
  ...
  if (aeron_cluster_members_parse(ctx->cluster_members, &a->active_members,
  ...
  if (NULL == a->this_member) {
    AERON_SET_ERR(EINVAL, "member_id %d not found in cluster_members",
                  a->member_id);
  ...
  /* Ranked positions for quorum — needs space for all members, not just quorum
   * threshold */
  ...
  for (int i = 0; i < a->service_count; i++) {
    a->service_ack_positions[i] = -1;
  }

  /* Service snapshot recording IDs — the relevant_id from each service's
   * snapshot ACK */
  ...
  for (int i = 0; i < a->service_count; i++) {
    a->service_snapshot_recording_ids[i] = -1;
  }
...
static void aeron_cm_logging_error_handler(void *clientd, int errcode,
                                           const char *message) {
  (void)clientd;
  fprintf(stderr, "WARNING: (%d): %s\n", errcode, message);
}

int aeron_consensus_module_agent_on_start(
    aeron_consensus_module_agent_t *agent) {
  aeron_cm_context_t *ctx = agent->ctx;

  /* Auto-create Aeron client if none was provided -- mirrors Java
   * ClusteredMediaDriver pattern where each component creates its own client to
  ...
  if (NULL == ctx->aeron) {
    ...
    if (aeron_init(&ctx->aeron, aeron_ctx) < 0 || aeron_start(ctx->aeron) < 0) {
      AERON_APPEND_ERR("%s", "");
      return -1;
    }
     ...
     * Override the default error handler (which calls exit) with the CM's
     * handler. */
    aeron_error_handler_t handler = NULL != ctx->error_handler
                                        ? ctx->error_handler
                                        : aeron_cm_logging_error_handler;
    void *handler_clientd =
        NULL != ctx->error_handler ? ctx->error_handler_clientd : NULL;
   ...
   * the caller's responsibility. The agent checks counters during do_work
   * instead. */

  /* Add subscriptions */
  /* Build subscription channels with this_member's endpoints appended —
   * mirrors Java ConsensusModuleAgent: consensusChannelUri.put(ENDPOINT,
   * thisMember.consensusEndpoint()) */
  char ingress_channel_full[512];
  char consensus_channel_full[512];
  if (NULL != agent->this_member &&
      NULL != agent->this_member->ingress_endpoint &&
    ...
    snprintf(ingress_channel_full, sizeof(ingress_channel_full),
             "%s%sendpoint=%s", ctx->ingress_channel, sep,
             agent->this_member->ingress_endpoint);
  } else {
    snprintf(ingress_channel_full, sizeof(ingress_channel_full), "%s",
             ctx->ingress_channel);
  }
  if (NULL != agent->this_member &&
      NULL != agent->this_member->consensus_endpoint &&
    ...
    snprintf(consensus_channel_full, sizeof(consensus_channel_full),
             "%s%sendpoint=%s", ctx->consensus_channel, sep,
             agent->this_member->consensus_endpoint);
  } else {
    snprintf(consensus_channel_full, sizeof(consensus_channel_full), "%s",
             ctx->consensus_channel);
  }

  if (add_sub(agent->aeron, &agent->ingress_subscription, ingress_channel_full,
              ctx->ingress_stream_id) < 0) {
    return -1;
  }

  if (add_sub(agent->aeron, &agent->consensus_subscription,
              consensus_channel_full, ctx->consensus_stream_id) < 0) {
    return -1;
  }

  if (add_sub(agent->aeron, &agent->service_sub, ctx->control_channel,
              ctx->consensus_module_stream_id) < 0) {
    return -1;
  }

  /* Add publications */
  if (add_exclusive_pub(agent->aeron, &agent->service_pub, ctx->control_channel,
                        ctx->service_stream_id) < 0) {
    return -1;
  }
  ...
  if (aeron_cluster_ingress_adapter_cm_create(
          ...
          AERON_CM_INGRESS_FRAGMENT_LIMIT_DEFAULT) < 0) {
    return -1;
  }

  if (aeron_cluster_consensus_adapter_create(&agent->consensus_adapter,
                                             agent->consensus_subscription,
                                             agent) < 0) {
    return -1;
  }
  ...
  if (aeron_cluster_log_adapter_create(
          &agent->log_adapter, agent,
          AERON_CLUSTER_LOG_ADAPTER_FRAGMENT_LIMIT) < 0) {
    return -1;
  }

  /* ConsensusModuleAdapter — polls service → CM subscription */
  if (aeron_cluster_consensus_module_adapter_create(
          &agent->cm_adapter, agent->service_sub, agent) < 0) {
    return -1;
  }

  /* Service proxy */
  aeron_cluster_service_proxy_cm_init(&agent->service_proxy, agent->service_pub,
                                      agent->service_count);

  /* Session manager */
  if (aeron_cluster_session_manager_create(&agent->session_manager,
  ...
  if (NULL != ctx->extension.on_session_opened) {
    agent->session_manager->on_session_opened_hook =
        ctx->extension.on_session_opened;
    agent->session_manager->on_session_closed_hook =
        ctx->extension.on_session_closed;
    agent->session_manager->extension_hook_clientd = ctx->extension.clientd;
  }

  /* Timer service */
  if (aeron_cluster_timer_service_create(&agent->timer_service, timer_on_expiry,
                                         agent) < 0) {
    return -1;
  }

  /* Recording log */
  if (aeron_cluster_recording_log_open(&agent->recording_log, ctx->cluster_dir,
                                       false) < 0) {
    if (aeron_cluster_recording_log_open(&agent->recording_log,
                                         ctx->cluster_dir, true) < 0) {
      return -1;
    }
  }
  ...
  if (NULL != ctx->archive_ctx) {
    ...
    aeron_archive_context_set_recording_signal_consumer(
        ctx->archive_ctx, cm_on_recording_signal, agent);
  ...
  if (aeron_alloc((void **)&agent->pending_trackers,
                  (size_t)agent->service_count *
                      sizeof(aeron_cluster_pending_message_tracker_t)) < 0) {
    AERON_APPEND_ERR("%s", "unable to allocate pending_trackers");
    return -1;
  }
  for (int i = 0; i < agent->service_count; i++) {
    aeron_cluster_pending_message_tracker_init(&agent->pending_trackers[i], i,
                                               1, 0, 4096);
  }
  ...
  int64_t leader_recording_id =
      (NULL != plan) ? plan->last_term_recording_id : -1;
  ...
  if (has_snapshot && NULL != agent->archive) {
    ...
    int replay_rc = aeron_archive_replay(
        &snap_sub, agent->archive, plan->snapshots[0].recording_id,
    ...
    if (replay_rc == 0 && NULL != snap_sub) {
      ...
      if (NULL != snap_image) {
        ...
        if (aeron_cluster_cm_snapshot_loader_create(
                &snap_loader, snap_image, agent->session_manager,
                agent->timer_service, agent->pending_trackers,
  ...
  /* Add consensus publications to all peers (mirrors Java
  ...
  if (aeron_cluster_members_add_consensus_publications(
          ...
          agent->aeron, ctx->consensus_channel, ctx->consensus_stream_id) < 0) {
    AERON_APPEND_ERR("%s", "failed to add consensus publications");
    return -1;
  }

  if (aeron_cluster_election_create(
          &agent->election, agent, agent->this_member, agent->active_members,
          agent->active_member_count, log_term_id, log_position, log_term_id,
          leader_recording_id, ctx->startup_canvass_timeout_ns,
          ctx->election_timeout_ns, ctx->election_status_interval_ns,
          ctx->leader_heartbeat_timeout_ns, true) < 0) {
    return -1;
  }
 ...
 * ----------------------------------------------------------------------- */
static int slow_tick_work(aeron_consensus_module_agent_t *agent,
                          int64_t now_ns) {
  int work_count = 0;

  /* Poll archive recording signals so the signal consumer can observe
   * START (to capture log_recording_id) and STOP (to trigger enterElection).
   * Mirrors Java ConsensusModuleAgent.consensusWork() → pollArchiveEvents().
   * NOTE: the C archive server currently only emits signals for replication
   * sessions (aeron_archive_replication_session.c), not for regular
   * recording START/STOP — so this poll is a no-op until that gap is
   * closed. Keeping the wiring in place so the consumer + flag path is
   * ready as soon as the server starts emitting. */
  if (NULL != agent->archive) {
    int32_t signal_count = 0;
    aeron_archive_poll_for_recording_signals(&signal_count, agent->archive);
    work_count += signal_count;
  }

  /* Mark file activity timestamp update (every ~1s).
   * Mirrors Java ConsensusModuleAgent.slowTickWork() →
   * markFile.updateActivityTimestamp(). */
  if (now_ns >= agent->mark_file_update_deadline_ns) {
    agent->mark_file_update_deadline_ns =
        now_ns + INT64_C(1000000000); /* 1 second */
  ...
  if (AERON_CLUSTER_ROLE_FOLLOWER == agent->role && NULL == agent->election &&
      (now_ns - agent->time_of_last_log_update_ns) >
          agent->leader_heartbeat_timeout_ns) {
    /* Start new election */
    if (aeron_cluster_election_create(
            &agent->election, agent, agent->this_member, agent->active_members,
            agent->active_member_count, agent->leadership_term_id,
            aeron_consensus_module_agent_get_append_position(agent),
            agent->leadership_term_id, agent->log_recording_id,
            agent->ctx->startup_canvass_timeout_ns,
            agent->ctx->election_timeout_ns,
            agent->ctx->election_status_interval_ns,
            agent->leader_heartbeat_timeout_ns, false) < 0) {
      return -1;
    }
  ...
  if (AERON_CLUSTER_ROLE_LEADER == agent->role && NULL == agent->election &&
    ...
    for (int i = 0; i < agent->active_member_count; i++) {
      if (agent->active_members[i].id != agent->member_id &&
        ...
        aeron_cluster_consensus_publisher_append_position(
            agent->active_members[i].publication, agent->leadership_term_id,
            pos, agent->member_id, 0);
      }
    }
  ...
  /* Leader: enter election if the local log recording stopped unexpectedly.
   * Mirrors Java ConsensusModuleAgent.onRecordingSignal(STOP) path — works
   * on single-node and multi-node since it triggers purely on local archive
   * state, not quorum membership. */
  if (AERON_CLUSTER_ROLE_LEADER == agent->role && NULL == agent->election &&
      agent->state == AERON_CM_STATE_ACTIVE &&
      agent->log_recording_stopped_unexpectedly) {
    agent->log_recording_stopped_unexpectedly = false;
    ...
    agent->role = AERON_CLUSTER_ROLE_FOLLOWER;
    if (aeron_cluster_election_create(
            &agent->election, agent, agent->this_member, agent->active_members,
            agent->active_member_count, agent->leadership_term_id,
            aeron_consensus_module_agent_get_append_position(agent),
  ...
  /* Leader: enter election if no quorum of active followers (more than 1-node
   * cluster) */
  if (AERON_CLUSTER_ROLE_LEADER == agent->role && NULL == agent->election &&
      agent->state == AERON_CM_STATE_ACTIVE && agent->active_member_count > 1 &&
      !aeron_cluster_members_has_active_quorum(
          agent->active_members, agent->active_member_count, now_ns,
          agent->leader_heartbeat_timeout_ns)) {
    ...
    /* Mirrors Java enterElection(): role(FOLLOWER) before creating the
     * election — without this a leader that has lost quorum still claims
     * the LEADER role while the election is in progress. */
    agent->role = AERON_CLUSTER_ROLE_FOLLOWER;
    if (aeron_cluster_election_create(
            &agent->election, agent, agent->this_member, agent->active_members,
            agent->active_member_count, agent->leadership_term_id,
  ...
  if (NULL != agent->ctx->extension.slow_tick_work) {
    work_count += agent->ctx->extension.slow_tick_work(
        agent->ctx->extension.clientd, now_ns);
  }
 ...
 */
static int check_cluster_control_toggle(aeron_consensus_module_agent_t *agent,
                                        int64_t now_ns) {
  if (NULL == agent->control_toggle_counter) {
    return 0;
  }
  ...
  case AERON_CLUSTER_TOGGLE_STANDBY_SNAPSHOT:
     ...
     * Full StandbySnapshotReplicator creation is a ClusterBackup Phase 4
     * feature; for now reset the toggle so we do not spin on it. */
    *toggle_addr = AERON_CLUSTER_TOGGLE_NEUTRAL;
    return 1;
...
static void on_session_timeout(void *clientd,
  ...
  if (NULL != agent->log_publication) {
    aeron_cluster_log_publisher_append_session_close(
        &agent->log_publisher, session->id, 2 /* TIMEOUT */,
        aeron_nano_clock());
  }
  ...
  if (NULL != session->response_publication) {
    aeron_cluster_egress_publisher_send_session_event(
        session->response_publication, session->id, session->correlation_id,
        agent->leadership_term_id, agent->member_id, 1 /* ERROR */,
        agent->leader_heartbeat_timeout_ns, "session timed out", 17);
  }
}
...
int aeron_consensus_module_agent_do_work(aeron_consensus_module_agent_t *agent,
  ...
  if (NULL != agent->consensus_adapter) {
    work_count +=
        aeron_cluster_consensus_adapter_poll(agent->consensus_adapter);
  }
  ...
  if (NULL != agent->session_manager) {
    ...
    if (AERON_CLUSTER_ROLE_LEADER == agent->role) {
      work_count += aeron_cluster_session_manager_process_pending_sessions(
          agent->session_manager, now_ns, now_ms, agent->member_id,
          agent->leadership_term_id);
      work_count += aeron_cluster_session_manager_send_rejections(
          agent->session_manager, agent->leadership_term_id, agent->member_id,
          now_ns);
       ...
       * The REDIRECT session event must carry the ELECTED leader's member_id
       * (not this follower's own member_id) so the client knows which
       * endpoint in the ingress_endpoints CSV to rebuild its publication
       * against. Mirrors Java ConsensusModuleAgent.slowTickWork passing
       * `leaderMember.id()` into sessionManager.checkSessions.
       * If leader_member is unknown (rare, e.g. in-election), fall back to
       * this node's id so the event still has a valid int32 — the client
       * will just not find a match and retry next REDIRECT. */
      const int32_t leader_id = (NULL != agent->leader_member)
          ? agent->leader_member->id : agent->member_id;
      work_count += aeron_cluster_session_manager_send_redirects(
          agent->session_manager, agent->leadership_term_id, leader_id,
          now_ns);
    }
  }
  ...
  if (AERON_CLUSTER_ROLE_LEADER == agent->role &&
      NULL != agent->timer_service) {
    work_count += aeron_cluster_timer_service_poll(
        agent->timer_service,
   ...
   * Mirrors Java: for (PendingServiceMessageTracker t :
   * pendingServiceMessageTrackers) t.poll()
   */
  if (AERON_CLUSTER_ROLE_LEADER == agent->role &&
      agent->state == AERON_CM_STATE_ACTIVE &&
    ...
    for (int i = 0; i < agent->service_count; i++) {
      work_count += aeron_cluster_pending_message_tracker_poll(
          &agent->pending_trackers[i], agent->leadership_term_id, now_ns,
          &agent->log_publisher, pending_service_message_log_append);
    }
  }
  ...
  if (AERON_CLUSTER_ROLE_LEADER == agent->role) {
    ...
    if (quorum_pos > agent->notified_commit_position ||
       ...
       * Mirrors Java sweepUncommittedEntriesTo ->
      ...
      while (agent->uncommitted_timers_count > 0 &&
        ...
        if (remaining > 0) {
          memmove(agent->uncommitted_timers, agent->uncommitted_timers + 2,
    ...
    /* Check cluster control toggle (SNAPSHOT / SHUTDOWN / ABORT / SUSPEND /
     * RESUME) */
    if (agent->state == AERON_CM_STATE_ACTIVE ||
        agent->state == AERON_CM_STATE_SUSPENDED) {
    ...
    /* While LEAVING, check if we can now terminate (all followers acked or
     * deadline) */
    if (agent->state == AERON_CM_STATE_TERMINATING &&
        agent->has_cluster_termination && can_terminate(agent, now_ns)) {
   ...
   * Mirrors Java: consensusWork → logAdapter.poll(min(notifiedCommitPosition,
   * limit)) If the image closes unexpectedly, enter election. */
  if (AERON_CLUSTER_ROLE_FOLLOWER == agent->role &&
      (agent->state == AERON_CM_STATE_ACTIVE ||
       agent->state == AERON_CM_STATE_SUSPENDED) &&
      NULL != agent->log_adapter && NULL == agent->election) {
    /* Bound poll by notified commit position so we don't consume ahead of
     * what's committed */
    int64_t poll_limit = agent->notified_commit_position;
    int log_poll =
        aeron_cluster_log_adapter_poll(agent->log_adapter, poll_limit);
    work_count += log_poll;
    ...
    if (NULL != agent->commit_position_counter) {
      ...
      if (NULL != cp && log_pos > *cp) {
        *cp = log_pos;
      }
    }
  }

  /* Follower: send AppendPosition to leader periodically or when position
   * advances. Mirrors Java: consensusWork → updateFollowerPosition(nowNs) */
  if (AERON_CLUSTER_ROLE_FOLLOWER == agent->role && NULL == agent->election &&
      NULL != agent->leader_member &&
      NULL != agent->leader_member->publication) {
    int64_t recorded_pos =
        aeron_consensus_module_agent_get_append_position(agent);
    int64_t pos = recorded_pos > agent->last_append_position
                      ? recorded_pos
    ...
    if (pos > agent->last_append_position ||
      ...
      if (aeron_cluster_consensus_publisher_append_position(
              agent->leader_member->publication, agent->leadership_term_id, pos,
              agent->member_id, 0)) {
        if (pos > agent->last_append_position) {
  ...
  if (AERON_CLUSTER_ROLE_FOLLOWER == agent->role &&
      (agent->state == AERON_CM_STATE_ACTIVE ||
       agent->state == AERON_CM_STATE_SUSPENDED) &&
      agent->termination_position >= 0 && NULL != agent->log_adapter) {
    int64_t log_pos = aeron_cluster_log_adapter_position(agent->log_adapter);
    if (log_pos >= agent->termination_position) {
      ...
      agent->state = AERON_CM_STATE_TERMINATING;
  ...
  if (NULL != agent->ctx->extension.do_work) {
    work_count +=
        agent->ctx->extension.do_work(agent->ctx->extension.clientd, now_ns);
  }
...
void aeron_consensus_module_agent_on_unavailable_counter(
  ...
  if (NULL != agent->ctx && NULL != agent->ctx->error_handler) {
    agent->ctx->error_handler(agent->ctx->error_handler_clientd, 0,
...
int aeron_consensus_module_agent_close(aeron_consensus_module_agent_t *agent) {
  if (NULL != agent) {
    ...
    if (NULL != agent->pending_trackers) {
      for (int i = 0; i < agent->service_count; i++) {
        aeron_cluster_pending_message_tracker_close(
    ...
    aeron_cluster_members_free(agent->active_members,
                               agent->active_member_count);
 ...
 * Accessors
 * ----------------------------------------------------------------------- */
int32_t aeron_consensus_module_agent_get_protocol_version(
    aeron_consensus_module_agent_t *a) {
  return a->protocol_version;
}

int32_t aeron_consensus_module_agent_get_app_version(
    aeron_consensus_module_agent_t *a) {
  return a->app_version;
}

int64_t aeron_consensus_module_agent_get_append_position(
    aeron_consensus_module_agent_t *a) {
...
int64_t aeron_consensus_module_agent_get_log_recording_id(
    aeron_consensus_module_agent_t *a) {
 ...
 * ----------------------------------------------------------------------- */
void aeron_consensus_module_agent_stop_all_catchups(
    aeron_consensus_module_agent_t *agent) {
  for (int i = 0; i < agent->active_member_count; i++) {
    ...
    if (m->catchup_replay_session_id != -1L) {
      if (m->catchup_replay_correlation_id != -1L && NULL != agent->archive) {
        if (aeron_archive_stop_replay(agent->archive,
                                      m->catchup_replay_session_id) < 0) {
...
int aeron_consensus_module_agent_truncate_log_entry(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t log_position) {
  if (NULL == agent->archive || agent->log_recording_id < 0) {
    return 0; /* nothing to truncate (e.g. follower with no archive) */
  }

  if (aeron_archive_stop_all_replays(agent->archive, agent->log_recording_id) <
      0) {
    return -1;
  }

  int64_t truncate_count = 0;
  if (aeron_archive_truncate_recording(&truncate_count, agent->archive,
...
void aeron_consensus_module_agent_on_election_state_change(
  ...
  aeron_cm_state_t cm_state = (new_state == AERON_ELECTION_INIT)
                                  ? AERON_CM_STATE_INIT
                                  : AERON_CM_STATE_ACTIVE;
...
void aeron_consensus_module_agent_on_election_complete(
    aeron_consensus_module_agent_t *agent, aeron_cluster_member_t *leader,
    int64_t now_ns, bool is_startup) {
  agent->leader_member = leader;
  agent->is_leader_startup = is_startup;
  /* updateMemberDetails: mark which member is leader, mirroring Java's
   * updateMemberDetails() */
  if (NULL != leader) {
    aeron_cluster_members_set_is_leader(agent->active_members,
                                        agent->active_member_count, leader->id);
  }
  bool is_leader = (leader != NULL && leader->id == agent->member_id);
  agent->role =
      is_leader ? AERON_CLUSTER_ROLE_LEADER : AERON_CLUSTER_ROLE_FOLLOWER;
  agent->time_of_last_log_update_ns = now_ns;

  /* prepareForNewLeadership: deactivate control toggle when stepping down from
   * leader. Mirrors Java ConsensusModuleAgent.prepareForNewLeadership(). */
   ...
   * fire again once a new leader is elected.  Mirrors Java
   * restoreUncommittedEntries(). */
  if (!is_leader && agent->uncommitted_timers_count > 0) {
    ...
    for (int i = 0; i < agent->uncommitted_timers_count; i++) {
      ...
      if (append_pos > commit_pos && NULL != agent->timer_service) {
        aeron_cluster_timer_service_schedule(agent->timer_service,
                                             correlation_id, 0);
      }
    }
  ...
  /* Restore pending service messages and sessions whose log entries were not
   * committed. Mirrors Java restoreUncommittedEntries() calls to
   * tracker.restoreUncommittedMessages() and
  ...
  if (NULL != agent->cluster_role_counter) {
    {
      int64_t *_cp = aeron_counter_addr(agent->cluster_role_counter);
      if (NULL != _cp)
        *_cp = (int64_t)agent->role;
    }
  }
  ...
  if (is_leader) {
    ...
    if (NULL == agent->log_publication) {
      ...
      aeron_exclusive_publication_constants(agent->log_publication,
                                            &pub_consts);
      agent->log_session_id_cache = pub_consts.session_id;

      aeron_cluster_log_publisher_init(&agent->log_publisher,
                                       agent->log_publication,
                                       agent->leadership_term_id);
      agent->log_publisher.aeron = agent->aeron;

      /* Wire log_publisher into session_manager so it can append session
       * open/close events */
      if (NULL != agent->session_manager) {
        agent->session_manager->log_publisher = &agent->log_publisher;
      }

      /* Start archive recording of the log (idempotent: join_log_as_leader
       * may have already created the recording on the LEADER_INIT path). */
      if (NULL != agent->archive && agent->log_subscription_id < 0) {
        aeron_archive_start_recording(
            &agent->log_subscription_id, agent->archive, ctx->log_channel,
            ctx->log_stream_id, AERON_ARCHIVE_SOURCE_LOCATION_LOCAL, false);
      }
    }
  }

  /* Update notified commit position to at least the election's final log
   * position. Mirrors Java ConsensusModuleAgent.electionComplete():
   *   notifiedCommitPosition = max(notifiedCommitPosition,
   * election.logPosition()) The election's log_position is the append position
   * at which the election resolved. */
  {
    int64_t elect_log_pos =
        (NULL != agent->election) ? agent->election->log_position : 0;
   ...
   * while (!ServiceAck.hasReached(logPosition, ...)) {
  ...
  if (agent->service_count > 0) {
    aeron_cluster_service_proxy_cm_join_log(
        &agent->service_proxy, append_pos, INT64_MAX, agent->member_id,
        agent->log_session_id_cache, ctx->log_stream_id,
        agent->is_leader_startup, (int32_t)agent->role, ctx->log_channel);
    ...
    if (NULL != agent->cm_adapter) {
      int64_t resend_deadline_ns =
          aeron_nano_clock() + INT64_C(200000000); /* 200ms */
      for (int spin = 0; spin < 50000; spin++) {
        aeron_cluster_consensus_module_adapter_poll(agent->cm_adapter);

        /* Check if service ACKed — mirrors Java
         * ServiceAck.hasReached(logPosition, ...) */
        bool all_acked = true;
...
void aeron_consensus_module_agent_begin_new_leadership_term(
    aeron_consensus_module_agent_t *agent, int64_t log_leadership_term_id,
    int64_t new_term_id, int64_t log_position, int64_t timestamp,
    bool is_startup) {
  agent->leadership_term_id = new_term_id;

  /* NOTE: recording.log is updated separately via update_recording_log →
   * ensure_coherent */

  /* Mark existing sessions so they receive a NewLeaderEvent.
   * On startup, sessions are closed (no existing clients); on recovery they
   * stay open. */
  if (NULL != agent->session_manager) {
...
void aeron_consensus_module_agent_on_follower_new_leadership_term(
    aeron_consensus_module_agent_t *agent, int64_t log_leadership_term_id,
    int64_t next_leadership_term_id, int64_t next_term_base_log_position,
    int64_t next_log_position, int64_t leadership_term_id,
    int64_t term_base_log_position, int64_t log_position,
    int64_t leader_recording_id, int64_t timestamp, int32_t leader_member_id,
    int32_t log_session_id, int32_t app_version, bool is_startup) {
...
void aeron_consensus_module_agent_on_replay_new_leadership_term_event(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t log_position, int64_t timestamp, int64_t term_base_log_position,
...
void aeron_consensus_module_agent_on_replay_session_message(
    aeron_consensus_module_agent_t *agent, int64_t cluster_session_id,
    int64_t timestamp) {
  /* Update last-activity for the session if we can find it */
  if (NULL != agent->session_manager) {
    aeron_cluster_cluster_session_t *session =
        aeron_cluster_session_manager_find(agent->session_manager,
                                           cluster_session_id);
    if (NULL != session) {
...
void aeron_consensus_module_agent_on_replay_timer_event(
    aeron_consensus_module_agent_t *agent, int64_t correlation_id) {
...
void aeron_consensus_module_agent_on_replay_session_open(
  ...
  if (NULL != agent->session_manager) {
    aeron_cluster_session_manager_on_replay_session_open(
        agent->session_manager, log_position, correlation_id,
        cluster_session_id, timestamp, response_stream_id, response_channel);
  }
}
...
void aeron_consensus_module_agent_on_replay_cluster_action(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t log_position, int64_t timestamp, int32_t action, int32_t flags) {
  ...
  if ((int32_t)aeron_cluster_client_clusterAction_SNAPSHOT == action &&
    ...
    if (NULL != agent->service_proxy.publication) {
      aeron_cluster_service_proxy_cm_request_service_ack(&agent->service_proxy,
                                                         log_position);
    }
  }
}
...
void aeron_consensus_module_agent_on_canvass_position(
  ...
  if (NULL != agent->election) {
    aeron_cluster_election_on_canvass_position(
        agent->election, log_leadership_term_id, log_position,
  ...
  if (NULL != m) {
     ...
     * Uses non-blocking async start — the publication will connect on a future
     * tick. Mirrors Java checkFollowerForConsensusPublication. */
    if (NULL == m->publication && AERON_CLUSTER_ROLE_LEADER == agent->role &&
      ...
      if (aeron_uri_string_builder_init_on_string(
              &builder, agent->ctx->consensus_channel) == 0) {
...
void aeron_consensus_module_agent_on_request_vote(
    aeron_consensus_module_agent_t *agent, int64_t log_leadership_term_id,
    int64_t log_position, int64_t candidate_term_id,
    int32_t candidate_member_id) {
  if (NULL != agent->election) {
    aeron_cluster_election_on_request_vote(
        agent->election, log_leadership_term_id, log_position,
...
void aeron_consensus_module_agent_on_vote(
    aeron_consensus_module_agent_t *agent, int64_t candidate_term_id,
    int64_t log_leadership_term_id, int64_t log_position,
    int32_t candidate_member_id, int32_t follower_member_id, bool vote) {
  if (NULL != agent->election) {
    aeron_cluster_election_on_vote(
        agent->election, candidate_term_id, log_leadership_term_id,
...
void aeron_consensus_module_agent_on_new_leadership_term(
    ...
    int64_t next_log_position, int64_t leadership_term_id,
    int64_t term_base_log_position, int64_t log_position,
    int64_t commit_position, int64_t leader_recording_id, int64_t timestamp,
    int32_t leader_member_id, int32_t log_session_id, int32_t app_version,
    bool is_startup) {
  ...
  if (0 != app_version && 0 != agent->app_version &&
    ...
    if (NULL != agent->ctx && NULL != agent->ctx->error_handler) {
      agent->ctx->error_handler(agent->ctx->error_handler_clientd, -1,
  ...
  if (NULL != agent->election) {
    aeron_cluster_election_on_new_leadership_term(
        agent->election, log_leadership_term_id, next_leadership_term_id,
        next_term_base_log_position, next_log_position, leadership_term_id,
        term_base_log_position, log_position, commit_position,
        leader_recording_id, timestamp, leader_member_id, log_session_id,
        app_version, is_startup);
    ...
    if (aeron_cluster_election_create(
            ...
            agent->leader_heartbeat_timeout_ns, false) < 0) {
      AERON_APPEND_ERR(
          "%s", "failed to create election on unexpected leadership term");
...
void aeron_consensus_module_agent_on_append_position(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t log_position, int32_t follower_member_id, int8_t flags) {
  aeron_cluster_member_t *m = aeron_cluster_member_find_by_id(
      agent->active_members, agent->active_member_count, follower_member_id);
  if (NULL != m) {
    m->log_position = log_position;
  ...
  if (NULL != agent->election) {
    aeron_cluster_election_on_append_position(agent->election,
                                              leadership_term_id, log_position,
                                              follower_member_id, flags);
  } else if (leadership_term_id <= agent->leadership_term_id &&
             AERON_CLUSTER_ROLE_LEADER == agent->role && NULL != m) {
    /* Outside of election: leader tracks catchup completion.
     * Mirrors Java ConsensusModuleAgent.trackCatchupCompletion(). */
    bool is_catchup_flag =
        (0 != (AERON_CLUSTER_APPEND_POSITION_FLAG_CATCHUP & flags));
    ...
    if (m->catchup_replay_session_id != -1 || is_catchup_flag) {
      ...
      if (m->log_position >= leader_pos) {
        ...
        if (m->catchup_replay_correlation_id != -1 && NULL != agent->archive) {
          aeron_archive_stop_replay(agent->archive,
                                    m->catchup_replay_session_id);
          m->catchup_replay_correlation_id = -1;
        }

        /* Send StopCatchup to the follower; clear session id when sent */
        if (m->catchup_replay_session_id != -1 && NULL != m->publication) {
          if (aeron_cluster_consensus_publisher_stop_catchup(
                  m->publication, agent->leadership_term_id,
                  follower_member_id)) {
            m->catchup_replay_session_id = -1;
          }
        }
...
void aeron_consensus_module_agent_on_commit_position(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t log_position, int32_t leader_member_id) {
  int64_t now_ns = agent->last_do_work_ns;
  ...
  if (NULL != agent->election) {
    aeron_cluster_election_on_commit_position(
        agent->election, leadership_term_id, log_position, leader_member_id);
  } else if (leadership_term_id == agent->leadership_term_id) {
    if (AERON_CLUSTER_ROLE_FOLLOWER == agent->role &&
        NULL != agent->leader_member &&
        leader_member_id == agent->leader_member->id) {
    ...
    if (aeron_cluster_election_create(
            ...
            agent->leader_heartbeat_timeout_ns, false) < 0) {
      AERON_APPEND_ERR(
          "%s", "failed to create election on unexpected commit position");
    } else if (NULL != agent->election) {
      agent->election->now_ns = now_ns;
      agent->election->time_of_state_change_ns = now_ns;
    }
...
void aeron_consensus_module_agent_on_catchup_position(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t log_position, int32_t follower_member_id,
  ...
  if (follower->catchup_replay_session_id != -1) {
    return;
  }

  /* Build a replay channel targeting the follower's catchup endpoint.
   * Mirrors Java: channel = ctx.followerCatchupChannel() with endpoint= and
  ...
  if (aeron_archive_start_replay(&replay_session_id, agent->archive,
                                 agent->log_recording_id, replay_channel,
                                 agent->ctx->log_stream_id, &params) >= 0) {
...
void aeron_consensus_module_agent_on_stop_catchup(
  ...
  /* This is received by the FOLLOWER when the leader wants to stop catchup
   * replay. On the follower side it removes the catchup log destination. On the
   * leader side it stops the archive replay for that follower. */
  if (AERON_CLUSTER_ROLE_LEADER == agent->role) {
    aeron_cluster_member_t *follower = aeron_cluster_member_find_by_id(
        agent->active_members, agent->active_member_count, follower_member_id);
    if (NULL != follower && follower->catchup_replay_session_id != -1) {
      if (NULL != agent->archive) {
        aeron_archive_stop_replay(agent->archive,
                                  follower->catchup_replay_session_id);
      }
    ...
    if (leadership_term_id == agent->leadership_term_id &&
        follower_member_id == agent->member_id && NULL != agent->log_adapter &&
...
void aeron_consensus_module_agent_on_termination_ack(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t log_position, int32_t member_id) {
  if (AERON_CLUSTER_ROLE_LEADER != agent->role) {
    return;
  }
...
void aeron_consensus_module_agent_on_session_connect(
    aeron_consensus_module_agent_t *agent, int64_t correlation_id,
    int32_t response_stream_id, int32_t version, const char *response_channel,
    const uint8_t *encoded_credentials, size_t credentials_length,
    aeron_header_t *header) {
  bool is_leader = (AERON_CLUSTER_ROLE_LEADER == agent->role);
  /* Follower REDIRECTs carry the full active-member ingress CSV
   * "id0=host:port,id1=host:port,..." — mirrors Java
   * ConsensusModuleAgent.slowTickWork passing
   * `ingressEndpoints = ClusterMember.ingressEndpoints(activeMembers)`.
   * The client's REDIRECT handler (aeron_cluster_async_connect) looks up
   * the leader's endpoint by `leader_member_id` in this CSV to rebuild the
   * ingress publication; sending only the single leader endpoint here left
   * the client unable to parse the detail. */
  char ingress_endpoints_buf[AERON_URI_MAX_LENGTH];
  const char *ingress_endpoints = NULL;
  if (!is_leader && NULL != agent->active_members && agent->active_member_count > 0) {
    if (aeron_cluster_members_ingress_endpoints(
            agent->active_members, agent->active_member_count,
            ingress_endpoints_buf, sizeof(ingress_endpoints_buf)) >= 0) {
      ingress_endpoints = ingress_endpoints_buf;
    }
  }

  aeron_cluster_session_manager_on_session_connect(
      agent->session_manager, correlation_id, response_stream_id, version,
      response_channel, encoded_credentials, credentials_length,
      aeron_nano_clock(), is_leader, ingress_endpoints);
}
...
void aeron_consensus_module_agent_on_session_close(
  ...
  /* Use session manager's proper close flow: appends to log + uncommitted
   * queue. The session is freed when services ACK the close position. */
  aeron_cluster_session_manager_on_session_close(
      agent->session_manager, cluster_session_id, leadership_term_id,
      aeron_nano_clock() / INT64_C(1000000));
}

void aeron_consensus_module_agent_on_session_keep_alive(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t cluster_session_id, aeron_header_t *header) {
  aeron_cluster_cluster_session_t *session = aeron_cluster_session_manager_find(
      agent->session_manager, cluster_session_id);
  if (NULL != session) {
...
void aeron_consensus_module_agent_on_session_message(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t cluster_session_id, const uint8_t *payload, size_t payload_length,
    aeron_header_t *header) {
  if (AERON_CLUSTER_ROLE_LEADER != agent->role) {
    return;
  }
  ...
  aeron_cluster_cluster_session_t *session = aeron_cluster_session_manager_find(
      agent->session_manager, cluster_session_id);
  if (NULL == session || session->state != AERON_CLUSTER_SESSION_STATE_OPEN) {
    return;
  }
  ...
  if (NULL != agent->log_publication) {
    ...
    int64_t pos = aeron_cluster_log_publisher_append_session_message(
        &agent->log_publisher, cluster_session_id, timestamp, payload,
        payload_length);
    if (pos > 0) {
      session->time_of_last_activity_ns = timestamp;
    }
...
void aeron_consensus_module_agent_on_ingress_challenge_response(
    aeron_consensus_module_agent_t *agent, int64_t correlation_id,
    int64_t cluster_session_id, const uint8_t *encoded_credentials,
    size_t credentials_length, aeron_header_t *header) {
  (void)header;
  if (AERON_CLUSTER_ROLE_LEADER == agent->role) {
    if (NULL != agent->session_manager) {
      aeron_cluster_session_manager_on_challenge_response(
          agent->session_manager, correlation_id, cluster_session_id,
          encoded_credentials, credentials_length, aeron_nano_clock());
    }
  } else {
    /* Follower: forward the challenge response to the leader via consensus
     * channel */
    if (NULL != agent->leader_member &&
        NULL != agent->leader_member->publication) {
      aeron_cluster_consensus_publisher_challenge_response(
          agent->leader_member->publication, correlation_id, cluster_session_id,
...
void aeron_consensus_module_agent_on_admin_request(
    aeron_consensus_module_agent_t *agent, int64_t leadership_term_id,
    int64_t cluster_session_id, int64_t correlation_id, int32_t request_type,
    const uint8_t *payload, size_t payload_length, aeron_header_t *header) {
  if (AERON_CLUSTER_ROLE_LEADER != agent->role) {
    return;
  }
  if (leadership_term_id != agent->leadership_term_id) {
  ...
  if (NULL != agent->session_manager) {
    session = aeron_cluster_session_manager_find(agent->session_manager,
                                                 cluster_session_id);
  }
  if (NULL == session || session->state != AERON_CLUSTER_SESSION_STATE_OPEN) {
    return;
  ...
  if (0 == request_type && agent->state == AERON_CM_STATE_ACTIVE &&
      NULL != agent->log_publication) {
    int64_t log_pos = aeron_consensus_module_agent_get_append_position(agent);
    int64_t timestamp = aeron_nano_clock();
    ...
    aeron_cluster_egress_publisher_send_admin_response(
        session->response_publication, cluster_session_id, correlation_id,
        request_type, 0 /* OK */, "");
  } else {
    /* Unknown or not applicable — respond ERROR */
    aeron_cluster_egress_publisher_send_admin_response(
        session->response_publication, cluster_session_id, correlation_id,
        request_type, 1 /* ERROR */, "request not applicable in current state");
  }
}
...
int64_t aeron_consensus_module_agent_quorum_position(
    aeron_consensus_module_agent_t *agent, int64_t append_position,
    int64_t now_ns) {
  ...
  for (int i = 0; i < n; i++) {
    /* Mirrors Java quorumPosition: inactive members (no recent heartbeat)
     * contribute -1 so they don't advance the quorum position. */
    if (aeron_cluster_member_is_quorum_candidate(
            &agent->active_members[i], now_ns,
            agent->leader_heartbeat_timeout_ns)) {
...
int aeron_consensus_module_agent_publish_new_leadership_term_on_interval(
    aeron_consensus_module_agent_t *agent, int64_t quorum_position,
    int64_t now_ns) {
  if ((now_ns - agent->time_of_last_log_update_ns) <
      agent->leader_heartbeat_interval_ns) {
    return 0;
  }
  agent->time_of_last_log_update_ns = now_ns;
  int64_t term_base =
      (agent->last_append_position >= 0) ? agent->last_append_position : 0;
  aeron_cluster_consensus_publisher_broadcast_new_leadership_term(
      agent->active_members, agent->active_member_count, agent->member_id,
      agent->leadership_term_id, agent->leadership_term_id, quorum_position,
      quorum_position, agent->leadership_term_id, term_base, quorum_position,
      agent->log_recording_id, now_ns, agent->member_id,
      agent->log_session_id_cache, agent->app_version, false);
  return 1;
}
...
int aeron_consensus_module_agent_publish_commit_position_on_interval(
    aeron_consensus_module_agent_t *agent, int64_t quorum_position,
    int64_t now_ns) {
  if ((now_ns - agent->time_of_last_append_position_send_ns) <
      agent->leader_heartbeat_interval_ns) {
    return 0;
  }
  agent->time_of_last_append_position_send_ns = now_ns;
...
aeron_cluster_log_replay_t *aeron_consensus_module_agent_new_log_replay(
    aeron_consensus_module_agent_t *agent, int64_t from_position,
    int64_t to_position) {
  if (NULL == agent->archive || agent->log_recording_id < 0 ||
      NULL == agent->ctx->log_channel) {
    return NULL;
  }
  aeron_cluster_log_replay_t *replay = NULL;
  if (aeron_cluster_log_replay_create(
          &replay, agent->archive, agent->aeron, agent->log_recording_id,
          from_position, to_position, agent->ctx->log_channel,
          agent->ctx->log_stream_id, agent->log_adapter) < 0) {
    return NULL;
  }
  return replay;
}

/* Scan the aeron counters for a RecordingPos counter (type_id=100) whose
 * key encodes a matching session_id, returning the encoded recording_id.
 * Returns -1 if no matching counter is visible yet. Mirrors Java
 * RecordingPos.findCounterIdBySession → RecordingPos.getRecordingId. */
typedef struct {
  int32_t target_session_id;
  int64_t found_recording_id;
} cm_find_recording_pos_ctx_t;

static void cm_find_recording_pos_cb(
    int64_t value, int32_t counter_id, int32_t type_id,
    const uint8_t *key, size_t key_length,
    const char *label, size_t label_length, void *clientd) {
  (void)value; (void)counter_id; (void)label; (void)label_length;
  cm_find_recording_pos_ctx_t *ctx = (cm_find_recording_pos_ctx_t *)clientd;
  if (100 != type_id) { return; }  /* AERON_COUNTER_ARCHIVE_RECORDING_POSITION_TYPE_ID */
  if (ctx->found_recording_id >= 0) { return; }
  /* Key layout (matches server's aeron_archive_conductor.c):
   *   recording_id (8) | session_id (4) | src_identity_len (4) | ... */
  if (key_length < 16) { return; }
  int64_t rec_id;
  int32_t sess_id;
  memcpy(&rec_id, key, 8);
  memcpy(&sess_id, key + 8, 4);
  if (sess_id == ctx->target_session_id) {
    ctx->found_recording_id = rec_id;
  }
}

static int64_t cm_find_recording_id_by_session_id(
    aeron_t *aeron, int32_t session_id) {
  aeron_counters_reader_t *reader = aeron_counters_reader(aeron);
  if (NULL == reader) { return -1; }
  cm_find_recording_pos_ctx_t ctx = { session_id, -1 };
  aeron_counters_reader_foreach_counter(reader, cm_find_recording_pos_cb, &ctx);
  return ctx.found_recording_id;
}

void aeron_consensus_module_agent_join_log_as_leader(
    aeron_consensus_module_agent_t *agent, int64_t term_id,
    int64_t log_position, int32_t session_id, bool is_startup) {
  (void)session_id;
  (void)is_startup;
  if (NULL == agent->log_publication && NULL != agent->ctx->log_channel) {
    if (add_exclusive_pub(agent->aeron, &agent->log_publication,
                          agent->ctx->log_channel,
                          agent->ctx->log_stream_id) < 0) {
      return;
    }
  }
  if (NULL == agent->log_publication) {
  ...
  agent->log_session_id_cache = -1;
  {
    ...
    if (aeron_exclusive_publication_constants(agent->log_publication,
                                              &consts) == 0) {
      agent->log_session_id_cache = consts.session_id;
    }
  }
  aeron_cluster_log_publisher_init(&agent->log_publisher,
                                   agent->log_publication, term_id);
  agent->log_publisher.aeron = agent->aeron;
  if (NULL != agent->session_manager) {
    agent->session_manager->log_publisher = &agent->log_publisher;
  }
  ...
  /* Start archive recording of the log stream, then block until the
   * RecordingPos counter (published by the archive once recording is
   * active) becomes visible with our log publication's session_id — at
   * that point we extract the real log_recording_id synchronously.
   *
   * Mirrors Java ConsensusModuleAgent.joinLogAsLeader + tryCreateAppendPosition:
   *   startLogRecording(...);
   *   while (!tryCreateAppendPosition(logSessionId)) { idle(); }
   *
   * Without this blocking wait, the immediately-following updateRecordingLog
   * would see log_recording_id == -1 and defer — leaving the recording_log
   * un-written until some later tick catches up, which produces flaky 3-node
   * cluster-client connect timeouts.
   *
   * The loop is bounded by the election timeout so a stuck archive can't
   * hang the election forever. */
  if (NULL != agent->archive && agent->log_subscription_id < 0) {
    aeron_archive_start_recording(
        &agent->log_subscription_id, agent->archive, agent->ctx->log_channel,
        agent->ctx->log_stream_id, AERON_ARCHIVE_SOURCE_LOCATION_LOCAL, false);
  }

  if (agent->log_recording_id < 0 && agent->log_session_id_cache >= 0 &&
      NULL != agent->aeron) {
    /* Synchronous best-effort: poll the RecordingPos counter for up to
     * 200 ms (mirrors Java joinLogAsLeader's tryCreateAppendPosition
     * loop; Java idles indefinitely but we can't since all CMs share the
     * test-thread tick — blocking here blocks peers). If the counter
     * doesn't resolve in time, the signal consumer (cm_on_recording_signal)
     * picks up the START signal on the next tick and flushes the
     * recording_log write. */
    const int64_t deadline_ns = aeron_nano_clock() + INT64_C(200000000);
    while (agent->log_recording_id < 0) {
      const int64_t rec = cm_find_recording_id_by_session_id(
          agent->aeron, agent->log_session_id_cache);
      if (rec >= 0) {
        agent->log_recording_id = rec;
        break;
      }
      if (aeron_nano_clock() >= deadline_ns) { break; }
...
void aeron_consensus_module_agent_update_recording_log(
  ...
  /* Java Election.ensureRecordingLogCoherent: bail on NULL recording_id as
   * a safety net for rare replication races. Normal leader path populates
   * log_recording_id synchronously in join_log_as_leader (via the
   * RecordingPos counter), so this guard rarely trips. */
  if (agent->log_recording_id < 0) {
    return;
  }
  ...
  aeron_cluster_recording_log_ensure_coherent(
      agent->recording_log, agent->log_recording_id,
      agent->initial_log_leadership_term_id,
      agent->initial_term_base_log_position, agent->leadership_term_id,
      term_base, -1, /* log_position: open (not yet committed) */
      now_ns);
}

int aeron_consensus_module_agent_update_leader_position(
    aeron_consensus_module_agent_t *agent, int64_t now_ns,
    int64_t append_position, int64_t quorum_position) {
  /* Keep leader's own member up-to-date for quorum calculations (mirrors Java
  ...
  if (NULL != agent->commit_position_counter &&
      quorum_position > agent->notified_commit_position) {
    int64_t *addr = aeron_counter_addr(agent->commit_position_counter);
    if (NULL != addr) {
      *addr = quorum_position;
    }
...
bool aeron_consensus_module_agent_append_new_leadership_term_event(
  ...
  if (NULL == agent->log_publication) {
    return true;
  } /* no log yet; skip gracefully */
  int64_t term_base =
      (agent->last_append_position >= 0) ? agent->last_append_position : 0;
  int64_t pos = aeron_cluster_log_publisher_append_new_leadership_term_event(
      &agent->log_publisher, agent->leadership_term_id,
      aeron_cluster_log_publisher_position(&agent->log_publisher), now_ns,
      term_base, agent->member_id, agent->log_session_id_cache,
      agent->app_version);
  return pos >= 0;
}

aeron_cluster_recording_replication_t *
aeron_consensus_module_agent_new_log_replication(
    aeron_consensus_module_agent_t *agent, const char *archive_endpoint,
    const char *resp_endpoint, int64_t leader_recording_id,
    int64_t stop_position, int64_t now_ns) {
  if (NULL == agent->archive || NULL == archive_endpoint) {
    return NULL;
  }

  char src_channel[512];
  snprintf(src_channel, sizeof(src_channel), "aeron:udp?endpoint=%s",
           archive_endpoint);
  ...
  params.dst_recording_id =
      agent->log_recording_id; /* extend existing recording */
  char resp_channel[512];
  if (NULL != resp_endpoint && *resp_endpoint != '\0') {
    snprintf(resp_channel, sizeof(resp_channel), "aeron:udp?endpoint=%s",
             resp_endpoint);
    params.src_response_channel = resp_channel;
  }

  aeron_cluster_recording_replication_t *rep = NULL;
  if (aeron_cluster_recording_replication_create(
          &rep, agent->archive, agent->aeron, leader_recording_id, src_channel,
          /* src_control_stream_id = */ 10, /* AeronArchive default control
                                               stream */
          &params, INT64_C(5) * INT64_C(1000000000), /* 5 s progress timeout */
...
void aeron_consensus_module_agent_update_recording_log_for_replication(
    aeron_consensus_module_agent_t *agent, int64_t term_id,
    int64_t base_position, int64_t stop_position, int64_t now_ns) {
  if (NULL == agent->recording_log) {
    return;
  }
  aeron_cluster_recording_log_ensure_coherent(
      agent->recording_log, agent->log_recording_id,
      agent->initial_log_leadership_term_id,
      agent->initial_term_base_log_position, term_id, base_position,
      stop_position > 0 ? stop_position : -1, now_ns);
}
...
void aeron_consensus_module_agent_add_catchup_log_destination(
    aeron_consensus_module_agent_t *agent, aeron_subscription_t *subscription,
    const char *endpoint) {
  if (NULL == subscription || NULL == endpoint) {
    return;
  }
  char dst[512];
  snprintf(dst, sizeof(dst), "aeron:udp?endpoint=%s", endpoint);
  aeron_subscription_async_add_destination(NULL, agent->aeron, subscription,
                                           dst);
  aeron_free(agent->catchup_log_destination);
...
bool aeron_consensus_module_agent_send_catchup_position(
  ...
  return aeron_cluster_consensus_publisher_catchup_position(
      agent->leader_member->publication, agent->leadership_term_id, pos,
      agent->member_id, catchup_endpoint);
}
...
bool aeron_consensus_module_agent_try_join_log_as_follower(
    aeron_consensus_module_agent_t *agent, aeron_image_t *image,
    bool is_startup, int64_t now_ns) {
  (void)is_startup;
  if (NULL == image || NULL == agent->log_adapter) {
    return false;
  }
...
int aeron_consensus_module_agent_catchup_poll(
  ...
  int fragments =
      aeron_cluster_log_adapter_poll(agent->log_adapter, commit_position);
  if (fragments > 0) {
    agent->last_append_position =
        aeron_cluster_log_adapter_position(agent->log_adapter);
    agent->time_of_last_log_update_ns = now_ns;
  }
  return fragments;
}
...
aeron_cm_state_t
aeron_consensus_module_agent_state(aeron_consensus_module_agent_t *agent) {
  return agent->state;
}
...
void aeron_consensus_module_agent_begin_termination(
  ...
  if (snapshot_first) {
    /* SHUTDOWN: take snapshot first, termination follows in
     * snapshotOnServiceAck */
    ...
    if (agent->service_count > 0) {
      aeron_cluster_service_proxy_cm_termination_position(&agent->service_proxy,
                                                          position);
    } else {
...
void aeron_consensus_module_agent_on_service_message(
  ...
  if (service_id >= 0 && service_id < agent->service_count &&
    ...
    aeron_cluster_pending_message_tracker_t *tracker =
        &agent->pending_trackers[service_id];
    if (!aeron_cluster_pending_message_tracker_should_append(
            tracker, cluster_session_id)) {
      return; /* already committed — skip */
    }

    int64_t pos = aeron_cluster_log_publisher_append_session_message(
        &agent->log_publisher, cluster_session_id, aeron_nano_clock(), payload,
        payload_length);
    if (pos > 0) {
      aeron_cluster_pending_message_tracker_on_appended(tracker,
                                                        cluster_session_id);
    }
  } else {
...
void aeron_consensus_module_agent_on_service_close_session(
  ...
  aeron_cluster_session_manager_remove(agent->session_manager,
                                       cluster_session_id);
}
...
static void timer_on_expiry(void *clientd, int64_t correlation_id) {
  ...
  int64_t cluster_time =
      (NULL != agent->ctx->cluster_clock_ns)
          ? agent->ctx->cluster_clock_ns(agent->ctx->cluster_clock_clientd)
          : aeron_nano_clock();
  ...
  if (append_position > 0) {
    ...
    if (agent->uncommitted_timers_count >= agent->uncommitted_timers_capacity) {
      int new_cap = (agent->uncommitted_timers_capacity == 0)
                        ? 16
                        : agent->uncommitted_timers_capacity * 2;
      int64_t *tmp = NULL;
      if (aeron_alloc((void **)&tmp, (size_t)(new_cap * 2) * sizeof(int64_t)) <
          0) {
        return;
      }
...
void aeron_consensus_module_agent_on_schedule_timer(
    ...
    int64_t deadline_ns) {
  if (NULL != agent->timer_service) {
    aeron_cluster_timer_service_schedule(agent->timer_service, correlation_id,
                                         deadline_ns);
  }
}
...
/*
 * CM takes its own snapshot: writes session + state data to the snapshot
 * channel via archive, appends the CM and per-service entries to the recording
 * log. service_relevant_ids[i] = the recording_id of service i's snapshot (from
 * ServiceAck).
 */
...
static void close_and_terminate(aeron_consensus_module_agent_t *agent) {
  ...
  if (agent->log_subscription_id >= 0 && NULL != agent->archive) {
    bool stopped = false;
    aeron_archive_try_stop_recording_subscription(&stopped, agent->archive,
                                                  agent->log_subscription_id);
...
/**
 * Set up a ClusterTermination and broadcast TerminationPosition to all
 * followers. Equivalent to new ClusterTermination(...) +
...
static void setup_cluster_termination(aeron_consensus_module_agent_t *agent,
  ...
  /* Reset has_terminate_notified on all members and broadcast
   * TerminationPosition to followers */
  for (int i = 0; i < agent->active_member_count; i++) {
...
static void do_termination_as_follower(aeron_consensus_module_agent_t *agent,
  ...
  if (agent->termination_leadership_term_id == agent->leadership_term_id &&
    ...
    aeron_cluster_consensus_publisher_termination_ack(
        agent->leader_member->publication, agent->leadership_term_id,
        log_position, agent->member_id);
  }
 ...
 * Equivalent to Java doTermination() when clusterTermination != null.
 */
static void do_termination_as_leader(aeron_consensus_module_agent_t *agent,
                                     int64_t log_position, int64_t now_ns) {
...
static void snapshot_timer_cb(void *clientd, int64_t correlation_id,
                              int64_t deadline_ns) {
  aeron_exclusive_publication_t *pub = (aeron_exclusive_publication_t *)clientd;
  aeron_cluster_cm_snapshot_taker_snapshot_timer(pub, correlation_id,
                                                 deadline_ns);
}

static void take_cm_snapshot(aeron_consensus_module_agent_t *agent,
                             int64_t log_position, int64_t timestamp,
                             int64_t *service_relevant_ids) {
  if (NULL == agent->archive || NULL == agent->recording_log) {
    return;
  }
  ...
  int32_t snap_session_id = -1;
  aeron_exclusive_publication_t *snap_pub = NULL;
  if (add_exclusive_pub(agent->aeron, &snap_pub, ctx->snapshot_channel,
                        ctx->snapshot_stream_id) == 0 &&
      NULL != snap_pub) {
    /* Capture the publication's session_id so we can look up the archive's
     * RecordingPos counter once recording has actually begun. */
    aeron_publication_constants_t snap_consts;
    if (aeron_exclusive_publication_constants(snap_pub, &snap_consts) == 0) {
      snap_session_id = snap_consts.session_id;
    }
    ...
    if (NULL != agent->session_manager) {
       ...
       * For multi-service setups, additional pendingMessageTracker records
       * follow. */
      ...
      if (NULL != agent->pending_trackers && agent->service_count > 0) {
        ...
        pending_cap =
            (int32_t)agent->pending_trackers[0].pending_message_capacity;
      }
      aeron_cluster_cm_snapshot_taker_snapshot_cm_state(
          snap_pub, agent->session_manager->next_session_id, next_svc_id,
          log_svc_id, pending_cap);

      /* Write per-service pendingMessageTracker records for service_count > 1
       */
      for (int i = 1;
           NULL != agent->pending_trackers && i < agent->service_count; i++) {
        aeron_cluster_cm_snapshot_taker_snapshot_pending_tracker(
            snap_pub, agent->pending_trackers[i].next_service_session_id,
            agent->pending_trackers[i].log_service_session_id,
            (int32_t)agent->pending_trackers[i].pending_message_capacity, i);
      }
    }
    ...
    if (NULL != agent->ctx->extension.on_take_snapshot) {
      agent->ctx->extension.on_take_snapshot(agent->ctx->extension.clientd,
                                             snap_pub);
    }
  ...
  if (snap_sub_id >= 0) {
    /* Resolve the snapshot recording_id via the RecordingPos counter keyed
     * by our snapshot publication's session_id, then wait for the archive
     * to actually finish writing the data (matching Java
     * ConsensusModuleAgent.takeSnapshot → awaitRecordingCounter +
     * awaitRecordingComplete). Java idles indefinitely; we use a generous
     * 5 s cap since CMs share a tick thread. */
    const int64_t snap_end_position =
        (NULL != snap_pub) ?
            aeron_exclusive_publication_position(snap_pub) : log_position;
    if (snap_session_id >= 0 && NULL != agent->aeron) {
      const int64_t deadline_ns = aeron_nano_clock() + INT64_C(5000000000);
      while (snap_recording_id < 0) {
        const int64_t rec = cm_find_recording_id_by_session_id(
            agent->aeron, snap_session_id);
        if (rec >= 0) { snap_recording_id = rec; break; }
        if (aeron_nano_clock() >= deadline_ns) { break; }
        aeron_micro_sleep(1000);
      }
      /* Wait until the archive's recording_position counter catches up to
       * the publication's stop position — otherwise the snapshot on disk
       * is truncated relative to what we wrote. Best-effort; if the
       * counter doesn't advance we break out after the deadline. */
      if (snap_recording_id >= 0) {
        while (true) {
          if (aeron_nano_clock() >= deadline_ns) { break; }
          int64_t recorded = -1;
          if (NULL != agent->archive) {
            aeron_archive_get_recording_position(
                &recorded, agent->archive, snap_recording_id);
          }
          if (recorded >= snap_end_position) { break; }
          aeron_micro_sleep(1000);
        }
      }
    }
  ...
  if (snap_recording_id < 0) {
    /* Couldn't resolve recording_id → snapshot entry is invalid. Skip the
     * recording_log append; the next tick will either retry snapshot or
     * proceed without one. Mirrors Java's throw-on-timeout behaviour (we
     * log + swallow rather than abort the CM since in-process we can't
     * easily unwind). */
    return;
  }

  /* Java ConsensusModuleAgent.takeSnapshot uses the current term's
   * termBaseLogPosition from the recording log (not log_position). Look
   * it up; fall back to log_position if the entry is missing. */
  int64_t term_base = log_position;
  if (NULL != agent->recording_log) {
    aeron_cluster_recording_log_entry_t *entry =
        aeron_cluster_recording_log_get_term_entry(
            agent->recording_log, agent->leadership_term_id);
    if (NULL != entry) {
      term_base = entry->term_base_log_position;
    }
  }
  aeron_cluster_recording_log_append_snapshot(
      agent->recording_log, snap_recording_id, agent->leadership_term_id,
  ...
  if (NULL != service_relevant_ids) {
    for (int i = 0; i < agent->service_count; i++) {
      aeron_cluster_recording_log_append_snapshot(
          agent->recording_log, service_relevant_ids[i],
          agent->leadership_term_id, term_base, log_position,
          timestamp / INT64_C(1000000), i /* service_id */);
    }
  }
}
...
void aeron_consensus_module_agent_on_service_ack(
    aeron_consensus_module_agent_t *agent, int64_t log_position,
    int64_t timestamp, int64_t ack_id, int64_t relevant_id,
    int32_t service_id) {
  (void)timestamp;
  if (service_id < 0 || service_id >= agent->service_count) {
    return;
  ...
  if (agent->state == AERON_CM_STATE_SNAPSHOT &&
    ...
    if (all_acked) {
      take_cm_snapshot(agent, agent->snapshot_log_position,
      ...
      /* If a ClusterTermination was in progress, transition to LEAVING and
       * notify services; otherwise return to ACTIVE. Mirrors Java
       * snapshotOnServiceAck(). */
      if (agent->has_cluster_termination) {
        if (agent->service_count > 0) {
          aeron_cluster_service_proxy_cm_termination_position(
  ...
  } else if (agent->state == AERON_CM_STATE_TERMINATING &&
             log_position == agent->expected_ack_position) {
    bool all_acked = true;
    for (int i = 0; i < agent->service_count; i++) {
      if (agent->service_ack_positions[i] != agent->expected_ack_position) {
  ...
  } else if (agent->state == AERON_CM_STATE_QUITTING) {
    /* Java: closeAndTerminate() throws ClusterTerminationException.
     * C equivalent: report error via error_handler and set state to CLOSED. */
    agent->state = AERON_CM_STATE_CLOSED;
    if (NULL != agent->ctx->error_handler) {
      agent->ctx->error_handler(
          agent->ctx->error_handler_clientd, 0,
          "cluster termination upon service ack in QUITTING state");
    }
  }
}
...
void aeron_consensus_module_agent_on_cluster_members_query(
  ...
  aeron_cluster_cluster_session_t *session = aeron_cluster_session_manager_find(
      agent->session_manager, correlation_id);

  if (NULL == session || AERON_CLUSTER_SESSION_STATE_OPEN != session->state) {
    return;
  }

  /* Build active members string: "id=ingressEndpoint,..." */
  char active_buf[4096];
  aeron_cluster_members_ingress_endpoints(agent->active_members,
                                          agent->active_member_count,
                                          active_buf, sizeof(active_buf));

  int32_t leader_id = (NULL != agent->leader_member) ? agent->leader_member->id
                                                     : agent->member_id;

  if (extended) {
    /* Extended response uses full "id,ep:ep:ep:ep:ep|..." format */
    char members_buf[8192];
    aeron_cluster_members_encode_as_string(agent->active_members,
                                           agent->active_member_count,
                                           members_buf, sizeof(members_buf));
    aeron_cluster_egress_publisher_send_cluster_members_response(
        session->response_publication, correlation_id, leader_id, members_buf,
        "");
  } else {
    aeron_cluster_egress_publisher_send_cluster_members_response(
        session->response_publication, correlation_id, leader_id, active_buf,
        "");
  }
}
...
void aeron_consensus_module_agent_on_backup_query(
    aeron_consensus_module_agent_t *agent, int64_t correlation_id,
    int32_t response_stream_id, int32_t version, int64_t log_position,
    const char *response_channel, const uint8_t *encoded_credentials,
    size_t credentials_length) {
  (void)log_position;
  ...
  if (NULL != agent->recording_log && agent->service_count > 0) {
    ...
    aeron_cluster_recording_log_find_snapshots_at_or_before(
        agent->recording_log,
        aeron_consensus_module_agent_get_append_position(agent),
        agent->service_count, raw, &raw_count);
    ...
    for (int i = 0; i < raw_count && snapshot_count < 32; i++) {
      ...
      snapshots[snapshot_count].term_base_log_position =
          raw[i].term_base_log_position;
  ...
  if (NULL != agent->aeron) {
    add_exclusive_pub(agent->aeron, &resp_pub, response_channel,
                      response_stream_id);
  }

  if (NULL != resp_pub) {
    aeron_cluster_consensus_publisher_backup_response(
        resp_pub, correlation_id, agent->log_recording_id,
        agent->leadership_term_id, log_term_base_position, commit_counter_id,
        leader_id, agent->member_id, snapshots, snapshot_count, members_buf);
...
void aeron_consensus_module_agent_on_heartbeat_request(
  ...
  /* Only the leader responds; only in ACTIVE/SUSPENDED/SNAPSHOT states; no
  ...
  if (NULL == response_channel || '\0' == response_channel[0]) {
    return;
  }

  /* Open a transient exclusive publication to the backup node's response
   * channel */
  aeron_exclusive_publication_t *resp_pub = NULL;
  if (NULL != agent->aeron) {
    add_exclusive_pub(agent->aeron, &resp_pub, response_channel,
  ...
  if (NULL != resp_pub) {
    aeron_cluster_consensus_publisher_heartbeat_response(resp_pub,
                                                         correlation_id);
    aeron_exclusive_publication_close(resp_pub, NULL, NULL);
  }
}
...
void aeron_consensus_module_agent_on_standby_snapshot(
    aeron_consensus_module_agent_t *agent, int64_t correlation_id,
    int32_t response_stream_id, int32_t version, const char *response_channel,
    int64_t *recording_ids, int64_t *leadership_term_ids,
    int64_t *term_base_log_positions, int64_t *log_positions,
    int64_t *timestamps, int32_t *service_ids, int snapshot_count) {
  (void)correlation_id;
  (void)response_stream_id;
  (void)version;
  ...
  aeron_cluster_session_manager_enqueue_standby_snapshot(
      agent->session_manager, recording_ids, leadership_term_ids,
      term_base_log_positions, log_positions, timestamps, service_ids,
      snapshot_count);
}

/* -----------------------------------------------------------------------
 * Extension message dispatch — mirrors Java
 * ConsensusModuleAgent.onExtensionMessage()
 * ----------------------------------------------------------------------- */
void aeron_consensus_module_agent_on_extension_message(
    aeron_consensus_module_agent_t *agent, int32_t acting_block_length,
    int32_t template_id, int32_t schema_id, int32_t acting_version,
    const uint8_t *buffer, size_t offset, size_t length) {
  if (NULL != agent->ctx->extension.on_ingress_extension_message &&
      agent->ctx->extension.supported_schema_id == schema_id) {
    agent->ctx->extension.on_ingress_extension_message(
        agent->ctx->extension.clientd, acting_block_length, template_id,
        schema_id, acting_version, buffer, offset, length);
  } else {
    /* No extension registered for this schema — report error.
     * Mirrors Java: countedErrorHandler.onError(new ClusterEvent(...)) */
    if (NULL != agent->ctx->error_handler) {
      agent->ctx->error_handler(agent->ctx->error_handler_clientd, EINVAL,
                                "unknown extension schema id");
    }
  }
}
